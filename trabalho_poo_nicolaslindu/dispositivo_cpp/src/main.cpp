...
// até 5 NTU → OK | até 10 NTU → ALERTA | acima de 10 NTU → FALHA
std::string statusTurbidez(double valor) {
    if (valor > 10.0) return "FALHA";
    if (valor >  5.0) return "ALERTA";
    return "OK";
}
...
int main() {
    ...
    std::cout << turbidez.getTag()    << ": " << vTurbidez    << " " << turbidez.getUnidade()    << std::endl;
    ...
    writer.escrever("AT-001", "turbidez",    vTurbidez,    turbidez.getUnidade(),    statusTurbidez(vTurbidez));
    ...
    writer.escrever("BBA-001", "bomba", 0.0, "estado", bombaFalha.getStatus());
